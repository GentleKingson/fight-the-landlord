...
| 🏠 房间系统  | 创建房间、加入房间、快速匹配                                                |
| 🤖 AI 机器人 | 凑人不足时自动填充空位；内置规则启发式机器人，可选接入 DouZero 神经网络引擎 |
...
stateDiagram-v2
    ...
    Ready --> Bidding : 发牌完成
    Bidding --> Bidding : 无人叫地主(流局重发)
    Bidding --> Playing : 叫抢结束，地主确定
    Playing --> GameOver : 有人出完牌（结算倍数）
    GameOver --> [*] : 解散
...
**代码位置**：`internal/game/match/matcher.go`
...
- 匹配逻辑：`internal/game/match/matcher.go`
- 游戏规则：`internal/game/rule/rule.go`
...
- [ ] 进一步增强规则机器人的出牌策略（已集成 DouZero 神经网络引擎作为可选强力对手）

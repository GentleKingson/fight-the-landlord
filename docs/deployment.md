...
当前 Web 客户端的重连凭证保存在 `localStorage`，但凭证有效期完全由服务端
时钟决定。连接保持在线期间，重连凭证不因墙钟时间过期；物理连接首次断开时
才启动完整 2 分钟恢复窗口，重复离线通知不会延长截止时间。成功恢复会单次消费
并轮换凭证；如果身份、房间或权威快照恢复失败，服务端会回滚轮换，旧凭证仍可在
原截止时间内重试。大厅的退出操作会先停止重连，再调用 `/session/revoke` 立即
撤销当前凭证。

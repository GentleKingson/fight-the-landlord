...
- `leaderboard:settlement:*` 结算幂等 key 仅作归档盘点，不作恢复下限。
...
每个 marker 从最后一名玩家结算起保留 30 天，显著长于正常重试、重连和 7 份
本地备份的窗口，且避免 key 数量随历史总牌局永久增长。超过 30 天的迟到重试
不再承诺去重；玩家统计和总榜仍为持久数据。
